...
#print (links_df)
#Write final links df to csv file.
links_df.to_csv('/Users/Marcia/OneDrive/MV2_Internship/All_Edges_Table.csv', index=False)
#Create nodes dataframe (where id=node name and group is defined for each node).
#Create a list of unique node id's from source and target labels.
source_df=network_df.copy()
source_df.rename(columns = {'source':'id'}, inplace=True)
source_df=source_df['id']
target_df=network_df.copy()
target_df.rename(columns = {'target':'id'}, inplace=True)
target_df=target_df['id']
all_nodes=pd.concat([source_df,target_df],axis=0)
...
nodes_df['group']=nodes_df.id.str[:1]
#print(nodes_df)
#Get a count of the unique group.
count = nodes_df.groupby(['group']).size().reset_index().rename(columns={0:'count'})
#print(count)
#Read in cleanup maingroup definitions. 
defs_df = pd.read_csv('/Users/Marcia/OneDrive/MV2_Internship/mg_defs.csv',
                         keep_default_na=False, na_values=[""])
defs_df.rename(columns = {'mg_def':'id_def'}, inplace=True)
#Merge the nodes with the definitions.
nodes_def_df = pd.merge(how='left', left=nodes_df,right=defs_df, left_on='id', right_on='maingroup')
nodes_def_df.drop('maingroup', axis=1, inplace=True)                         
#Save nodes df to csv file.
nodes_def_df.to_csv('/Users/Marcia/OneDrive/MV2_Internship/Def_Nodes_Table.csv', index=False)
